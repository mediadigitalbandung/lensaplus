...
Format output WAJIB JSON valid (tanpa teks lain, tanpa code-fence):
{
  ...
  // Category: prefer keyword-level override, fall back to source category.
  const categoryId = kw.categoryId || source.categoryId;

  // Safe featured image: extractFirstImageUrl strips data: URIs automatically.
  // Fall back to raw field only when it is a plain http(s) URL.
  const safeImageUrl =
    extractFirstImageUrl(source.featuredImage || "") ||
    (source.featuredImage && !source.featuredImage.startsWith("data:")
      ? source.featuredImage
      : null);
  const featuredImage = safeImageUrl;
  ...
  // on the public page). URL and alt text are HTML-attribute-escaped to
  // prevent injection. The featuredImage field is also kept for OG tags
  ...
  if (featuredImage && !/<img[^>]*src=/i.test(bodyHtml)) {
    bodyHtml =
      `<p><img src="${escapeHtmlAttr(featuredImage)}" alt="${escapeHtmlAttr(parsed.title.slice(0, 200))}" /></p>` +
      bodyHtml;
  }

  // Create article — sourceArticleId always set (we abort earlier if no source).
  const slug = await uniqueSlug(parsed.title);
  // Auto-article DRAFTs are always UNVERIFIED. Guardrail warnings are reflected
  // in reviewNote, not the label (label is for editor-reviewed status).
  const verificationLabel = "UNVERIFIED" as const;
  ...
  // Persist suggestedTags (best-effort — tag failures must not abort article creation).
  if (Array.isArray(parsed.suggestedTags) && parsed.suggestedTags.length > 0) {
    const tagsToConnect = parsed.suggestedTags
      .filter((t: unknown): t is string => typeof t === "string" && t.trim().length > 0)
      .slice(0, 8)
      .map((name: string) => ({ name: name.trim(), slug: slugify(name.trim()) }));

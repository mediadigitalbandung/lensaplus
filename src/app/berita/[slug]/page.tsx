              ...
              {article.featuredImage && (
                <div className="group relative">
                  ...
                  {(() => {
                    ...
                    if (article.isAutoGenerated) {
                      const lowerCredit = (featuredImageCredit || "").toLowerCase();
                      if (!displayCredit || lowerCredit.includes("kartawarta")) {
                        // 1. Try to get source name from article.sources
                        if (article.sources && article.sources.length > 0) {
                          displayCredit = article.sources[0].name;
                        } else {
                          // 2. Try to parse from content
                          const linkMatch = article.content.match(/<a\s+[^>]*href=["'](https?:\/\/[^"']+)["'][^>]*>([\s\S]*?)<\/a>/i);
                          if (linkMatch) {
                            const url = linkMatch[1];
                            const text = linkMatch[2].replace(/<[^>]+>/g, "").trim();
                            if (text && !text.toLowerCase().includes("kartawarta")) {
                              const dashIdx = text.indexOf(" —");
                              displayCredit = dashIdx > 0 ? text.substring(0, dashIdx).trim() : text.trim();
                            } else {
                              try {
                                displayCredit = new URL(url).hostname.replace("www.", "");
                              } catch {
                                displayCredit = null;
                              }
                            }

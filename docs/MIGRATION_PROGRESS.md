# Lensaplus Feature Migration — Progress Tracker

> **Project:** Lensaplus v2.0. Sumber referensi fitur: `docs/FEATURE_REFERENCE.md` (diambil dari dokumen master JHB).
...
- [x] Deploy ke VPS: `git pull && prisma generate && prisma db push && npm run build && pm2 restart lensaplus` — sukses 2026-04-24, PM2 `lensaplus` online
...
- [x] **H-4 FIXED**: `/api/og` `loadBackground` allowlist host (lensaplus.com + subdomain + Unsplash + Meta CDN + 145.79.15.99)
...
- **Deploy ke VPS**: user jalankan `git pull && prisma generate && prisma db push && npm run build && pm2 restart lensaplus` — sukses. PM2 `lensaplus` online.
...
- 100% migrasi fitur Lensaplus sesuai `docs/FEATURE_REFERENCE.md` selesai

🎉 **MIGRASI FITUR LENSAPLUS SELESAI 13/13 FASE.**
...
- **Brand:** Tetap Lensaplus (lensaplus.com). Dokumentasi menyebut JHB karena sumbernya dari jurnalishukumbandung.com. Dalam kode, pakai constant `src/lib/constants.ts` untuk domain/brand.
- **Role:** Lensaplus pakai 6 role (bukan 4 seperti dokumentasi). Editor group = `{CHIEF_EDITOR, EDITOR}`. Writer group = `{SENIOR_JOURNALIST, JOURNALIST, CONTRIBUTOR}`. Sudah ada helper `canPublishDirectly`, `canApproveArticles`, `canWriteArticles`, `canManageUsers`, `canManageAds` di [src/lib/auth.ts](../src/lib/auth.ts).
...
- **PM2 process name:** `lensaplus` (bukan `jhb`).
- **App dir VPS:** `/var/www/lensaplus`.
